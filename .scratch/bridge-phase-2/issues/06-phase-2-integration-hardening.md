...
- [ ] `bun test packages/bridge` passes full suite including Phase 2 integration tests
- [ ] At least one CLI integration test each for `bridge log` and `bridge status`
- [ ] CursorTeam dispatch covered by integration test with mocked SDK
- [ ] README documents Phase 2 commands and `SCOTTY_CURSOR_*` / `CURSOR_API_KEY` env vars per `.env-template`
- [ ] `CONTEXT.md` reflects Phase 2 MVP command set
- [ ] No tests make real Cursor API, Claude, or Discord network calls
