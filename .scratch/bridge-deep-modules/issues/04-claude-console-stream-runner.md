...
- [ ] `runClaudePrompt` handles PATH resolution, streaming, duration, and exit code → success mapping
- [ ] `SCOTTY_CLAUDE_PATH` overrides PATH for both Claude Team and Diagnostic Cycle agent
- [ ] Missing `claude` binary throws `ClaudeNotFoundError` before spawn
- [ ] Console stream UX preserved: stdout/stderr reach terminal in real time during dispatch and diagnostic
- [ ] Claude Team executes against target repo path; Diagnostic agent against Scotty Vault path
- [ ] `ExecutionResult` shape unchanged (`success`, `stdout`, `stderr`, `durationMs`)
- [ ] Runner tests cover path resolution, stream teeing, and success/failure without real `claude` binary
