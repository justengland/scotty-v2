...
- [x] `bridge dispatch <repo> --issue path/to/issue.md` builds Task from issue markdown
- [x] Title from H1; description from **What to build** section
- [x] Mutually exclusive with `--title`/`--description`; clear error if both provided
- [x] Full dispatch pipeline runs (integration test with mock Away Team)
- [x] Works with repos using `claude-code` or `cursor` agent profiles
