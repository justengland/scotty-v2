...
- [ ] Mission Orders `agent = "cursor"` routes dispatch to CursorTeam
- [ ] Missing `CURSOR_API_KEY` aborts before SDK call with clear error
- [ ] Model, streaming, runtime, and node bin read from `SCOTTY_CURSOR_*` env vars per `.env-template`
- [ ] Task prompt includes title, description, and injected Archive context (same shape as Claude Team)
- [ ] Tricorder gate, Engineering Log append, and failure hails behave identically to Claude Team dispatch
- [ ] Dispatch integration test with mocked SDK; no network in CI
- [ ] `.env-template` remains the canonical env reference (no duplicate env docs with conflicting names)
